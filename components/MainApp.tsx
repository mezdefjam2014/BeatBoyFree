...
import { 
  ...
  Clock, Briefcase, Volume2, Globe, Megaphone, ExternalLink
} from 'lucide-react';
import Knob from './Knob';
import Waveform from './Waveform';
import ProducerTools from './ProducerTools';
import MessageTicker from './MessageTicker';
import GlobalMessageScreen from './GlobalMessageScreen';
import AdImageScreen from './AdImageScreen';
import { AudioFile, EQSettings, TagSettings, VideoSettings, ColorGradeType, VisualizerType, Theme, AdData } from '../types';
...
// Fallback Ad Data to ensure display if fetch fails
const FALLBACK_AD: AdData = {
    "ad_image": "https://www.sleek-mag.com/wp-content/uploads/2016/08/AlbumCovers_Blonde-1200x1200.jpg",
    "ad_link": "https://phonicore.com",
    "ad_title": "Tap to Learn More",
    "aspect_ratio": "1:1"
};
...
const MainApp: React.FC<MainAppProps> = ({ onLock }) => {
  ...
  const [adGlow, setAdGlow] = useState(false);
  ...
  const [masterVolume, setMasterVolume] = useState(0.85); // 0.0 to 1.0
  ...
  const masterMeterRef = useRef<HTMLCanvasElement>(null);
  const masterMeterReqRef = useRef<number>(0);
  ...
  useEffect(() => {
    const fetchAd = async () => {
        try {
            // Updated URL to point to the latest raw gist file to ensure real-time updates without commit hash.
            const url = `https://gist.githubusercontent.com/mezdefjam2014/3ca1d13995c703b881784cef31c81110/raw/gistfile1.txt?t=${Date.now()}`;
            const res = await fetch(url);
            
            if (res.ok) {
                const newData: AdData = await res.json();
                
                setAdData(prev => {
                    // Check if content actually changed to trigger glow
                    if (prev && (prev.ad_image !== newData.ad_image || prev.ad_title !== newData.ad_title)) {
                        setAdGlow(true);
                        showSuccessToast("New Offer", "A new sponsored offer is available.");
                    }
                    return newData;
                });
            } else {
                // If fetch fails, use fallback if we have no data yet
                setAdData(prev => prev || FALLBACK_AD);
            }
        } catch (e) {
            // Network error, ensure at least fallback is shown
            setAdData(prev => prev || FALLBACK_AD);
        }
    };
    ...
    const interval = setInterval(fetchAd, 15000);
  ...
  useEffect(() => {
    ...
    analyserRef.current.fftSize = 2048; // Increased for better meter resolution
  ...
  // Update Gain when Master Volume changes
  useEffect(() => {
      if (gainNodeRef.current && audioCtxRef.current) {
          gainNodeRef.current.gain.setTargetAtTime(masterVolume, audioCtxRef.current.currentTime, 0.05);
      }
  }, [masterVolume]);
  ...
  // --- Realtime Master Meter Visualizer ---
  useEffect(() => {
      const canvas = masterMeterRef.current;
      if (!canvas) return;
      ...
      const drawMasterMeter = () => {
          ...
          if (analyserRef.current) {
              ...
              for (let i = 0; i < bufferLength; i++) {
                  ...
                  // Gradient color based on height/magnitude
                  const hue = (i / bufferLength) * 360; // Rainbow-ish
                  // Or standard level meter: green -> yellow -> red
                  const percent = barHeight / height;
                  let fillStyle = '#10b981'; // Green
                  if (percent > 0.6) fillStyle = '#fbbf24'; // Yellow
                  if (percent > 0.85) fillStyle = '#ef4444'; // Red
          ...
          masterMeterReqRef.current = requestAnimationFrame(drawMasterMeter);
      };
      ...
      return () => cancelAnimationFrame(masterMeterReqRef.current);
  }, []); // Run once on mount
  ...
  const drawVideoFrame = (ctx: CanvasRenderingContext2D, width: number, height: number, time: number, dataArray: Uint8Array, timeDataArray: Uint8Array) => {
     ...
     // ... (rest of video logic remains same, removed for brevity as requested by diff logic unless changed)
     // To satisfy "Full content", I must include the rest.
     // Re-adding the rest of drawVideoFrame content
  ...
  const generateSocialVideo = async () => {
    // ... (logic is same, keeping for structure, implementation in prior blocks)
    // Re-implemented to ensure no lost code.
     if (!activeFile?.buffer) return;
     stopAudio();
  ...
  const handlePreviewPointerDown = (e: React.PointerEvent) => {
     ...
     const hits = videoSettings.overlays
         ...
         .map(o => ({
             ...
             // Larger hit radius for the price card because it's a wide rectangle, not just center-aligned text
             threshold: o.id === 'price' ? 0.3 : 0.15
         }))
         .filter(h => h.dist < h.threshold) 
  ...
  return (
    ...
    >
      ...
      {showGlobalMessage && (
          <GlobalMessageScreen onClose={() => setShowGlobalMessage(false)} />
      )}

      {showAdScreen && (
          <AdImageScreen onClose={() => { setShowAdScreen(false); setAdGlow(false); }} data={adData || FALLBACK_AD} />
      )}
      ...
      <div className="w-72 bg-[var(--bg-panel)] border-r border-[var(--border)] flex flex-col relative z-20 shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
        ...
        {adData && (
            <div className="px-4 pt-2 bg-zinc-900/50 backdrop-blur-md">
                <div 
                    onClick={() => adData.ad_link && window.open(adData.ad_link, '_blank')}
                    className="relative w-full aspect-square rounded-xl overflow-hidden border border-zinc-700 cursor-pointer group hover:border-[var(--accent)] transition-all shadow-lg"
                >
                    <img 
                        src={adData.ad_image} 
                        alt={adData.ad_title} 
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                        <div className="bg-[var(--accent)] text-black text-[8px] font-black px-1.5 py-0.5 rounded w-fit mb-1">AD</div>
                        <p className="text-xs font-bold text-white leading-tight shadow-black drop-shadow-md">{adData.ad_title}</p>
                    </div>
                </div>
            </div>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[var(--bg-main)] relative shadow-inner overflow-hidden">
        ...
        <div className="h-40 border-b border-[var(--border)] bg-gradient-to-b from-[#121214] to-[#0c0c0e] flex items-center justify-center px-8 shadow-md relative z-10 shrink-0">
           ...
           <div className="flex items-center mr-8 gap-4">
               <div className="flex items-center gap-2">
                   <div onClick={() => setShowGlobalMessage(true)} className="cursor-pointer">
                        <MessageTicker />
                   </div>
                   {/* Sponsored button removed as requested */}
               </div>
               <BPMButton />
           </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="p-6 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:h-56 h-auto shrink-0">
        ...
        {/* MASTER OUTPUT & METERS (FIXED BOTTOM) */}
        <div className="shrink-0 p-6 border-t border-[var(--border)] bg-[#0c0c0e] relative z-20">
            <div className="h-64 bg-[#121214] border border-[var(--border)] rounded-xl p-4 flex gap-4 shadow-2xl relative overflow-hidden">
                <div className="flex-1 relative bg-black rounded-lg overflow-hidden border border-zinc-800 shadow-inner group">
                    <canvas ref={masterMeterRef} className="w-full h-full opacity-80 group-hover:opacity-100 transition-opacity" />
                    <div className="absolute top-2 left-2 flex items-center gap-2">
                        <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-red-500/50 shadow-lg"/>
                        <span className="text-[9px] font-bold text-zinc-500 tracking-widest uppercase">Master Output Spectrum</span>
                    </div>
                    {/* Grid Overlay */}
                ...
                <div className="w-20 bg-zinc-900 rounded-lg border border-zinc-800 p-2 flex flex-col items-center justify-between relative shadow-inner">
                    <div className="text-[9px] font-black text-zinc-500 uppercase tracking-widest text-center mb-1">Main Mix</div>
                    
                    <div className="relative flex-1 w-full flex justify-center py-2">
                        {/* Fader Track */}
                        <div className="absolute top-2 bottom-2 w-1.5 bg-black rounded-full border border-zinc-800 shadow-inner left-1/2 -translate-x-1/2" />
                        
                        {/* Tick Marks */}
                        <div className="absolute top-2 bottom-2 left-1 flex flex-col justify-between text-[8px] text-zinc-600 font-mono pointer-events-none">
                            <span>+6</span>
                            <span>0</span>
                            <span>-inf</span>
                        </div>
                        ...
                        <input 
                            ...
                            className="opacity-0 cursor-pointer absolute inset-0 z-20 appearance-none"
                            style={{ 
                        ...
                        {/* Visual Thumb */}
                        <div 
                            className="absolute left-1/2 -translate-x-1/2 w-10 h-6 bg-gradient-to-b from-zinc-700 to-zinc-800 rounded shadow-lg border-t border-zinc-600 border-b border-zinc-950 z-10 pointer-events-none flex items-center justify-center group-hover:bg-zinc-600 transition-colors"
                            style={{ bottom: `${(masterVolume / 1.2) * 80}%`, transition: 'bottom 0.05s linear' }}
                        >
                            <div className="w-full h-0.5 bg-zinc-950/50 mb-0.5" />
                            <div className="w-full h-0.5 bg-white shadow-[0_0_5px_white]" />
                            <div className="w-full h-0.5 bg-zinc-950/50 mt-0.5" />
                        </div>
                    </div>

                    <div className="mt-1 flex items-center gap-1 bg-black/50 px-1.5 py-0.5 rounded border border-white/5">
                        <Volume2 size={10} className={masterVolume > 0 ? "text-[var(--accent)]" : "text-zinc-600"} />
                        <span className="text-[9px] font-mono text-[var(--accent)] w-6 text-right">
                        ...
                        {videoSettings.previewUrl ? (
                            <div className="flex gap-2">
                                <a 
                                    ...
                                    download={`BEATBOY_${activeFile?.name || 'VIDEO'}.webm`}
                                    className="flex-1 bg-green-500 hover:bg-green-400 text-black font-bold py-3 rounded-lg shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"
                                >
                                    <Download size={16} /> SAVE VIDEO
                                </a>
                                <button 
                                    onClick={() => setVideoSettings(p => ({...p, previewUrl: null}))}
                                    className="px-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg"
                                >
                                    <X size={16} />
                                </button>
                            </div>
                            ...
                            <button 
                                ...
                                className="w-full bg-[var(--accent)] hover:opacity-90 text-black font-bold py-3 rounded-lg shadow-lg shadow-[var(--accent-dim)] transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                                {videoSettings.isGenerating ? <Loader2 className="animate-spin" size={16} /> : <Film size={16} />}
                                {videoSettings.isGenerating ? 'RENDERING...' : 'RENDER VIDEO'}
                            </button>
                        )}
                    </div>
                </div>
            ...
            {videoTab === 'assets' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                     ...
                     {videoSettings.logoUrl && (
                        <div className="relative h-20 bg-zinc-900 rounded-lg border border-zinc-800 flex items-center justify-center group overflow-hidden">
                             <img src={videoSettings.logoUrl} className="max-h-16 max-w-[80%] object-contain" alt="Logo" />
                             <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                 <button 
                                     onClick={() => setVideoSettings(p => ({...p, logoUrl: null}))}
                                     className="p-1.5 bg-[var(--accent)] hover:opacity-90 rounded-full text-white"
                                     title="Remove Logo"
                                 >
                                     <X size={14} />
                                 </button>
                     ...
                     >
                        ...
                        <span className="text-[9px] font-bold text-zinc-500">DRAG CUSTOM FONT (.TTF)</span>
                     </div>
                     <p className="text-[9px] text-zinc-600 text-center font-mono">{videoSettings.fontFamily}</p>
                </div>
            )}
         </div>
      </div>
